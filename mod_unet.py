...
class NewUnet_FD(nn.Module): # NewUnet with Fractional Downsampling
    def __init__(self, in_chans: int, out_chans: int, chans: int = 64, num_pool_layers: int = 4, drop_prob: list = None):
        """
        Args:
            in_chans: Number of channels in the input to the U-Net model.
            out_chans: Number of channels in the output to the U-Net model.
            chans: Number of output channels of the first convolution layer.
            num_pool_layers: Number of down-sampling and up-sampling layers.
            drop_prob: A list of dropout probabilities.
        """
        super().__init__()

        if drop_prob is None:
            drop_prob = [0.0] * (num_pool_layers + 1)  # Default to no dropout if not specified
        elif len(drop_prob) != num_pool_layers + 1:
            raise ValueError("drop_probs must have the same length as num_pool_layers + 1")
    ...
    def forward(self, image: torch.Tensor) -> torch.Tensor:
        ...
        # Apply down-sampling layers with fractional scaling
        for layer in self.down_sample_layers:
            output = layer(output)
            stack.append(output)
            output = F.interpolate(output, scale_factor=0.25, mode='bilinear', align_corners=True)

        output = self.conv(output)

        # Apply up-sampling layers with fractional scaling
        for transpose_conv, conv in zip(self.up_transpose_conv, self.up_conv):
            downsample_layer = stack.pop()
            output = F.interpolate(output, scale_factor=4.0, mode='bilinear', align_corners=True)
